...
static TR::Register* inlineIntrinsicStringIndexOfString(TR::Node* node, TR::CodeGenerator* cg, bool isLatin1)
   ...
   TR_ASSERT(comp->target().is64Bit(), "Not supported on 32-bit platform");
   ...
   */
   ...
   generateRegMemInstruction(isLatin1 ? TR::InstOpCode::L1RegMem : TR::InstOpCode::L2RegMem, node, tmpReg, generateX86MemoryReference(s2Reg, hdrSize, cg), cg);
   ...
   if (isLatin1)
      ...
      {
      generateRegMemInstruction(TR::InstOpCode::L2RegMem, node, tmpReg, generateX86MemoryReference(s2Reg, s2idxReg, 1, hdrSize, cg), cg);
      generateMemRegInstruction(TR::InstOpCode::CMP2MemReg, node, generateX86MemoryReference(s1Reg, s1idxReg, 1, hdrSize, cg), tmpReg, cg);
      }
   generateLabelInstruction(TR::InstOpCode::JNE1, node, unmatchLabel, cg);
