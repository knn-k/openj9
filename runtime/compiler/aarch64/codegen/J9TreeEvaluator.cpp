...
static TR::Register* inlineIntrinsicIndexOf(TR::Node* node, TR::CodeGenerator* cg, bool isLatin1)
   ...
   if (TR::Compiler->om.isOffHeapAllocationEnabled())
      ...
      generateTrg1MemInstruction(cg, TR::InstOpCode::ldrimmx, node, dataAddrReg, TR::MemoryReference::createWithDisplacement(cg, arrayReg, dataAddrOffset));
      }
   else
#endif /* J9VM_GC_SPARSE_HEAP_ALLOCATION */
      {
      uint32_t hdrSize = static_cast<uint32_t>(TR::Compiler->om.contiguousArrayHeaderSizeInBytes());
      generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addimmx, node, dataAddrReg, arrayReg, hdrSize);
      }

   // Character to find
   generateTrg1Src1Instruction(cg, isLatin1 ? TR::InstOpCode::vdup16b : TR::InstOpCode::vdup8h, node, vtmp2Reg, charReg);

   if (!offsetNode->isConstZeroValue())
      {
      // @@ zero-extend to 64 bits
      if (isLatin1)
         {
         generateTrg1Src2Instruction(cg, TR::InstOpCode::addx, node, tmp1Reg, dataAddrReg, resultReg);
         }
      else
         {
         generateTrg1Src2ShiftedInstruction(cg, TR::InstOpCode::addx, node, tmp1Reg, dataAddrReg, resultReg, TR::SH_LSL, 1);
         }
      }
   ...
   generateTrg1Src2Instruction(cg, TR::InstOpCode::lsrvx, node, tmp1Reg, tmp1Reg, tmp3Reg); // @@ type of the third reg?
   ...
   if (isLatin1)
      ...
      generateTrg1MemInstruction(cg, TR::InstOpCode::vldroffq, node, vtmp1Reg, TR::MemoryReference::createWithIndexReg(cg, dataAddrReg, tmp1Reg)); // @@ use LSL ?
      }
   generateTrg1Src2Instruction(cg, isLatin1 ? TR::InstOpCode::vcmeq16b : TR::InstOpCode::vcmeq8h, node,
                               vtmp1Reg, vtmp1Reg, vtmp2Reg);
   if (isLatin1)
