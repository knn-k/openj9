...
TR_PersistentJittedBodyInfo *J9::Recompilation::getJittedBodyInfoFromPC(void *startPC)
   ...
   if (linkageInfo->isSamplingMethodBody())
      ...
      if ((binst & B_COND_INSTR_MASK) == (TR::InstOpCode::getOpCodeBinaryEncoding(TR::InstOpCode::b_cond) | TR::CC_LT)) // blt instruction to snippet
         {
...
void J9::Recompilation::methodHasBeenRecompiled(void *oldStartPC, void *newStartPC, TR_FrontEnd *fe) // @@ done except for some constants
   {
   ...
   // Code Cache Reclamation does not work on aarch64 @@
...
void J9::Recompilation::methodCannotBeRecompiled(void *oldStartPC, TR_FrontEnd *fe) // @@ done except for some constants
   {
   J9::PrivateLinkage::LinkageInfo *linkageInfo = J9::PrivateLinkage::LinkageInfo::get(oldStartPC);
   TR_J9VMBase *fej9 = (TR_J9VMBase *)fe;
   int32_t *patchAddr, newInstr;
   ...
   if (bodyInfo->getUsesPreexistence()
      ...
      *patchAddr = encodeDistanceInBranchInstruction(TR::InstOpCode::b, distance);
      ...
      newInstr = encodeDistanceInBranchInstruction(TR::InstOpCode::b, distance);
      *patchAddr = newInstr;
      arm64CodeSync((uint8_t *)patchAddr, ARM64_INSTRUCTION_LENGTH);
      if (DEBUG_ARM64_RECOMP)
         {
