...
#if 0
   TR::SymbolReference *boundsCheckSR = comp()->getSymRefTab()->methodSymRefFromName(
      comp()->getMethodSymbol(),
      "java/lang/String", "checkBoundsOffCount", "(III)V",
      TR::MethodSymbol::Static
      );
   TR::Node *boundsCheckNode = TR::Node::createWithSymRef(node, TR::call, 3, boundsCheckSR);
   TR::Node *arrlenNode = TR::Node::create(node, TR::arraylength, 1, dstObj);
   boundsCheckNode->setAndIncChild(0, dstOff);
   boundsCheckNode->setAndIncChild(1, length);
   boundsCheckNode->setAndIncChild(2, arrlenNode);
#endif
...
#if 0
   treetop->insertBefore(TR::TreeTop::create(comp(), boundsCheckNode));
#endif
