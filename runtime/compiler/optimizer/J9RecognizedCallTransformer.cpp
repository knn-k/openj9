...
void J9::RecognizedCallTransformer::process_java_lang_StringLatin1_inflate_BIBII(TR::TreeTop *treetop, TR::Node *node)
   ...
   TR::Node *srcOff = node->getChild(1)->createLongIfNeeded();
   TR::Node *dstObj = node->getChild(2);
   TR::Node *dstOff = node->getChild(3)->createLongIfNeeded();
   ...
   TR::Node *constZeroNode1 = TR::Node::create(TR::iconst, 0, 0);
   ...
   TR::Node *constZeroNode2 = TR::Node::create(TR::iconst, 0, 0);
   ...
   TR::Node *isubNode = TR::Node::create(TR::isub, 2, length, arrayLenNode);
   ...
   // Split the current block right after the ificmplt
   ...
   TR_ASSERT_FATAL(treetop->getNextTreeTop(), "treetop->getNextTreeTop() must be non-NULL");
   TR::Block *tailBlock = fallbackPathBlock->split(treetop->getNextTreeTop(), cfg, true /* fixUpCommoning */, true /* copyExceptionSuccessors */);

   // Now create a node to go to the tail block.
   TR::Node *gotoNode = TR::Node::create(node, TR::Goto);
   TR::TreeTop *gotoTree = TR::TreeTop::create(comp(), gotoNode, NULL, NULL);
   gotoNode->setBranchDestination(tailBlock->getEntry());
   fallThroughPathBlock->getExit()->insertBefore(gotoTree);

   // Now we have fall-through block, fallback block and tail block.
   // Set the ificmp's destination to the fallback block and update the CFG as well.
